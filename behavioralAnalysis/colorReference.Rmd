...
listener_df %>% 
  ...
  mutate(improvement = ifelse(!literal & pragmatic, 1, 0),
         decline = ifelse(literal & !pragmatic, 1, 0),
         nochange = ifelse(literal == pragmatic, 1, 0)) %>%
  group_by(condition) %>%
  summarize("improvedM" = mean(improvement), 
            "declinedM" = mean(decline),
            "se" = .5 * sqrt(1/length(improvement)),
            "nochangeM" = mean(nochange)
            ) %>%
  gather(change, count, ends_with("M")) %>%
  mutate(change = gsub(".$", "", change)) %>%
  ggplot(aes(x = condition, y = count, fill = change)) +
    geom_bar(stat = 'identity', position = dodge) +
    geom_errorbar(aes(ymax = count + se, ymin = count - se),
                  position=dodge, width=0.25) +
    theme_bw() +
    ylab("% of trials changed L0->L2")
```
