...
workerIds = read.csv("./humanOutput/colorReferenceWorkerIDs.csv") 
...
Next, create our inclusion criterion (remove games where participants were confused, spoke english as a second language, or didn't complete all trials) and create a list of the games that satisfy it.
...
Why are some of the unique worker ids NAs? Because there was a small number of turkers (`{r} length(unique(combined_human %>% filter(is.na(workerid_uniq))))`) that completed the task but did not submit their HIT. We're including this data even though we don't have complete subject info. 

```{r}
length(unique(combined_human %>% filter(is.na(workerid_uniq))))
```
...
paste(c(length(unique(combined_human$gameid)), 
        ...
        length(unique(rawAggregated$gameid))), collapse = " ")
```
...
# Print out summary of listener performance
listener_df %>% group_by(condition, source) %>%
