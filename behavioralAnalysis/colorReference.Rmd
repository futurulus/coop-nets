...
subjInfo = read.csv("./humanOutput/colorReference-subject_information_batch2.csv") %>%
  select(-score) %>%
  rbind(read.csv("./humanOutput/colorReference-subject_information_batch1.csv")) %>%
...
```{r}
robo_msgs = read.csv("./modelOutput/speaker_big_sl_sampled_message.csv") %>%
  rename(msgTime = time, role = sender) %>%
  mutate(role = "speaker") %>%
  mutate(contents = trimws(as.character(contents)))

robo_clks = read.csv("./modelOutput/listener_big_l2_untuned_clickedObj.csv") %>%
  mutate(condition = factor(condition, levels = c("closer", "further", "equal"), 
                            labels = c("close", "split", "far"))) %>%
  rename(clkTime = time)
...
(taggedDF %>% filter(gameid == "4261-9"))$contents
(combined %>% filter(gameid == "4261-9"))$contents

# TODO: present se in table (e.g. in parens after mean)
resultTable = combined %>% 
  left_join(taggedDF) %>%
  #filter(!is.na(numComp)) %>% # TODO: figure out what's going wrong with joining
