...
length(unique(workerIds$workerid_uniq))
```
...
paste(c(length(unique((combined_human %>% filter(is.na(workerid_uniq)))$gameid)),
        "games where we don't have complete subject info"), collapse = ' ')

setdiff(unique(clks$gameid), unique(workerIds$gameid))
```
...
paste(c("total of", length((combined_human %>% filter(role == "speaker"))$gameid), "utterances"),
      collapse = ' ')
```
