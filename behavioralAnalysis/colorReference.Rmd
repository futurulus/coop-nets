...
```{r}
robo_msgs = read.csv("./modelOutput/speaker_reccontext_tuned_sampled_message.csv") %>%
...
(taggedDF %>% filter(gameid == "4927-d"))$contents
...
resultTable = combined %>% 
  ...
  mutate(comb = ordered(comb, 
                        levels = c('human_far', 'human_split', 'human_close',
                                  'model_far', 'model_split', 'model_close'))) %>%
  spread(comb, mu)
...
topRow <- paste0(paste0('& \\multicolumn{3}{c}{', 
                        ...
                        '}', collapse=''), '\\\\')
secondRow <- paste0('& ', 
                    paste0(rep(unique(combined$condition), 2), collapse='& '),
                    '\\\\')
