...
\# Chars & 8.17 & 11.87 & 13.12 \\ 
  \# Comparatives & 0.01 & 0.06 & 0.06 \\ 
  \# Negatives & 0.05 & 0.09 & 0.11 \\ 
  \# Superlatives & 0.03 & 0.07 & 0.16 \\ 
...
\noindent{\bf Comparatives and superlatives:} We used the StanfordNLP POS tagger to count the number of comparatives (JJR or RBR) and superlatives (JJS or RBS) per message. We found that participants were more likely to use these constructions when one or more distractors were close to the target. Additionally, we found evidence of an asymmetry in the use of these constructions across the \emph{split} and \emph{close} conditions. Comparatives were used equally often in both conditions, while superlatives were twice as likely to be used when all colors were equally close. \todo[inline]{Note that these results were actually computed using the terrible built-in nltk tagger because the stanford nlp server is down; it counts ``forest green'' as a superlative and thinks that ``brightest'' is a noun, so take it with a grain of salt.}
