...
The corpus includes 948 complete games with 53,365 utterances produced by
...
\textcolor{red}{rdh: recent RSA papers have switched to using $l_1$ as notation for the pragmatic listener that reasons about $s_1$ instead of going up a number with every recursion, although I can also see how it's useful to distinguish between this $l_1$ and the $l_1$ that reasons about a literal speaker later\dots}
\begin{align}
  l_{2}(\target \| \utt, \Lex)
  &\propto
...
a literal speaker:
\textcolor{red}{rdh: notation for lexicon is different here than earlier; should it be $\Lex(\utt, \target)$ instead of suggesting conditional probability?}
\begin{align}
  s_{0}(\utt \| \target, \Lex)
  &\propto
...
\textcolor{red}{rdh: if this shift in notation from lower-case to capital letters is intentional, maybe put a sentence that explicitly signposts this at the end of the previous section?}
