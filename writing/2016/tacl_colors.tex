...
(\tabref{table:metrics}). In all cases, we report test statistics from a mixed-effects regression including condition as a fixed effect and game ID as a random effect; due to the relatively large corpus, all test statistics reported correspond to $p$-values $<~10^{-5}$ and have been omitted.
