...
\begin{figure*}[t]
  ...
  \end{subfigure}
  %
  \caption{An example of RSA applied to an idealized color reference task.}
  \label{fig:basic-rsa}
\end{figure*}
...
\cite{Karpathy2015,Vinyals2015}, in using a second recurrent neural network for producing
...
\end{multline}
%
\todocheck{I think people might expect $0 \leqslant \beta \leqslant
  1$. However, we evidentially tested a different range. Let's say
  here what values $\beta$ can have.} This allows the model to adjust
the extent to which it overrides the learned model with the pragmatic
reasoning. A base weight $\beta = 0$ makes the blended model
equivalent to the pragmatic model, and $\beta = 1$ ignores the
pragmatic reasoning and uses the base model's outputs.
...
Since color reference is a difficult task even for humans, we compared listener accuracy across conditions to calibrate our expectations about model performance. While participants' accuracy was close to ceiling (97\%) on the `far' condition, they made significantly more errors on the `split' (90\%) and `close' (83\%) conditions (see \todocheck{[make new figure/table for this]}).
...
We expect human speakers to be verbose in `close' contexts; even if
...
and the `far', `split`, and `close` contexts. \todocheck{Regression
  for this too?}
...
constructions across the \emph{split} and \emph{close} conditions.
Comparatives were used somewhat more often in the `split' condition,
where only one distractor was close to the target, while superlatives
were much more likely to be used in the `close' condition.
...
\emph{close} conditions will tend to lead speakers to go lower in this
hierarchy. \todocheck{To do... and include comparison with our $\Speaker_{1}$.} \cite{Fellbaum1998}
