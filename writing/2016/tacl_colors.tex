...
\todocheck{(cite)} and Adam \todocheck{(cite)}, adaptive variants of
...
on a held-out tuning set consisting of 3,500 contexts.\footnote{For
  $\Speaker_0$: Adam, learning rate $\alpha = {}$0.004; for
  $\Listener_0$: Adadelta, learning rate $\eta = {}$0.2.}
...
\begin{tabular}{lrrrrrr}
  ...
  $\Listener_1$ & \best{84.11} & \best{84.82} & \oracle{\todocheck{$>$99}} & \best{1.49} & \best{1.45} & \oracle{\todocheck{$<$1.1}} 
  \\[0.5ex]
...
\todocheck{We would also mention that we didn't model the sequential nature
of the utterances in the corpus. This is a liability for us: it might
confuse readers that we didn't do it, given the nature of the corpus, 
and they might criticize us for not doing it. A strong concluding note
on this (perhaps foreshadowed somewhere early in the paper) could help
with this issue.}
