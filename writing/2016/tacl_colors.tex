...
Both of these versions of RSA unfortunately pose problems with
...
$\Lex$ can be factorized in an unrealistically clean way.
...
various blends ($A \cdot B$ denotes an weighted average of $A$ and $B$).
...
pragmatic models with the listener also improves over both individually. Finally, the
...
Breaking out the number of examples changed by condition on the dev set
...
reference games, and we showed that a pragmatic reasoning agent
incorporating neural models of color description understanding and production
is more accurate at
context-dependent description understanding than the neural understanding
model alone.
...
top of pre-trained base models.
