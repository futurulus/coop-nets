...
\todocheck{Maybe call this section ``pragmatic agents'' since we also define the prag speaker here, which we refer back to later?}
...
We split our corpus into equal train/dev/test splits (\todocheck{16,635} contexts per split)
...
$m = 2$ and $n = 1$ (see \secref{sec:l2}). \todocheck{rdh: maybe put a note about what the boxes mean? It looks like it's meant to facilitate visual comparison between boxes, but I think it's actually intended to mark the intended referent of the message? If the latter, why is only the first box marked in the $S_1$ row?}}
...
target color to the listener models. \todocheck{rdh: can we make these either both refer to sections or both refer to equations?}
...
the difference between $\Listener_0$ and $\Listener^2$ (\todocheck{superscript typo}): the pragmatic model,
