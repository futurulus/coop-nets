...
utterances as speakers and interpreting the utterances we hear as listeners.
Dialogues involving color
...
In this paper, we present a scalable model of pragmatic language understanding.
...
\begin{figure*}[t]
  ...
  \begin{subfigure}[t]{0.3\textwidth}
    ...
    \toprule
     & \colorContext{3884C7}{02F9FD}{9E6461}{} \\
    \midrule
  ...
  \end{subfigure}
  %
  \caption{An example of RSA applied to color reference.}
  \label{fig:basic-rsa}
\end{figure*}
...
of a \term{literal listener} (\figref{fig:basic-rsa:l0}):
\begin{align}
  l_{0}(\target \| \utt, \Lex)
  &\propto
...
Note that the $\alpha$ parameter of the speaker indirectly affects the
...
$\Messages$ and the interpretation functon $\Lex$. In most versions of
...
\cite{Karpathy2015,Vinyals2015}, but using a second recurrent neural network for producing
...
\begin{tabular}{lrrr|rrr}
  & \multicolumn{3}{c}{human}& \multicolumn{3}{c}{pragmatic}\\ \hline
  ...
  \% Superlatives & 0.02 & 0.06 & 0.16 & 0.05 & 0.10 & 0.17 \\ 
   \hline
\end{tabular}
\caption{Corpus statistics and statistics of samples from $\Speaker_1$ (rates per utterance). \todocheck{todo: check human words rates, put errors in?}}
\label{table:metrics}
\end{table*}
...
Since color reference is a difficult task even for humans, we compared listener accuracy across conditions to calibrate our expectations about model performance. While participant accuracy was close to ceiling (97\%) on the `far' condition, they made significantly more errors on the `split' (90\%) and `close' (83\%) conditions (see \todocheck{[make new figure/table for this]}).
...
lengthy modifier phrases anyway. To assess this hypothesis, we
...
Additionally, we replace the multi-layer perceptron models over $n$-gram features
used in \newcite{AndreasKlein16_NeuralPragmatics} with more effective
recurrent models for both the base listener and the base speaker; in particular,
our speaker is more expressive in that it is able to capture properties of
the context when sampling alternative utterances.
...
($\Speaker_0^{\beta} \cdot \Listener_0^{1 - \beta}$) and
