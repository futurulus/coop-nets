...
\cite{Hawkins15_RealTimeWebExperiments}.  \todocheck{After non-native
...
model is defined as follows, where $\state$ is a color in the context
set $\Colors$, $\msg$ is a message drawn from a set of messages
$\Messages$, $\StatePrior$ is a prior over colors, $\Costs$ is a
real-valued cost function on utterances, $\alpha \in [0,\infty)$ is an
inverse temperature parameter, and $\Lex(\msg, \state) = 1$ if $\msg$
is true of $\state$, else $0$:
%
\begin{align}
  l_{0}(\state \| \msg, \Lex) 
  &\propto 
  \Lex(\msg, \state) \StatePrior(\state) 
  \\
  s_{1}(\msg \| \state, \Lex)
  &\propto 
  e^{\alpha\log(l_{0}(\state \| \msg, \Lex)) - \Costs(\msg)} 
  \label{eq:rsa-s1}\\
  l_{0}(\state \| \msg, \Lex) 
  &\propto 
  s_{1}(\msg \| \state, \Lex) \StatePrior(\state) 
\end{align}
...
a listener who reasons about $\Lex$ directly. This back-and-forth
interpretive process mirrors the original definition of conversational
implicature \cite{Grice75} and reflects more general ideas from
Bayesian cognitive modeling \cite{Tenenbaum-etal:2011}. The model and
its variants have been shown to capture a wide range of pragmatic
phenomena in a cognitively realistic manner
\cite{Goodman:Stuhlmuller:2013,Smith:Goodman:Frank:2013,Kao-etal:2014,Bergen:Levy:Goodman:2014},
and the central Bayesian calculation has proven useful in a variety of
communicative domains
\cite{Tellex-etal:2014,Vogel:Potts:Jurafsky:2013}.
...
one would expect from a compositional semantic system), then there is
...
utterances that let us approximate the challengeing normalization
required for (\ref{eq:rsa-s1}).
